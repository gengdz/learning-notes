...
`performance.timing` 包含了从浏览器开始导航到当前页面加载完成的各个阶段的时间。
...
1. 对于一些较大类型的第三方库，比如 antd、lodash 等，设置 **按需加载**。
   一般都是使用 Babel 插件来实现 比如 _babel-plugin-import_
2. 通过**前端路由懒加载**的方式优化
...
1. 使用[熊猫站](https://tinypng.com/)可以无损压缩
   ...
   > 补充：
   >
   > 通过相似颜色量化的技术来减少颜色数量，并将 24 位的 PNG 文件转化成 8 位的彩色图片。同时可以将不必要的元数据进行剥离。
