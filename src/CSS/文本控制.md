...
span {
  font:
    bold italic 20px/1.5em 'Courier New',
    Courier,
    monospace;
}
```
...
一、`word-break`
决定了单词在到达边界时，如何“被砍断”以换行。

是否在文本溢出内容框的地方出现换行符。它有几个值：`normal | break-all | keep-all | break-word`

```css
word-break: break-word;
```

二、`overflow-wrap`(旧称：`word-wrap`)
当一个单词实在太长，长到一行放不下时，是否允许它在内部断开。

设置浏览器是否应该在不可破坏的字符串中出插入换行符，以防止文本溢出其行框。它有几个值：`normal | break-word`。
