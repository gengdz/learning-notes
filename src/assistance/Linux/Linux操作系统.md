...
- **网络接口管理**：可以显示、启用、禁用网络接口，以及设置接口的 MTU（最大传输单元）、混杂模式等。
- **地址配置**：为网络接口添加、删除或显示 IPv4 和 IPv6 地址配置。
- **路由管理**：查看、添加、删除路由条目，管理路由表，支持多路径路由（ECMP）和策略路由。
- **链接管理**：操作和显示链路层信息，如设置链路状态、修改接口名称等。
- **邻居管理**：显示和操作 ARP（Address Resolution Protocol）缓存。
- **隧道和通道**：配置和管理各种类型的 IP 隧道，如 GRE、IPIP、SIT 等。
- **策略和规则**：设置复杂的路由策略和规则，用于基于源地址、目的地址等条件的路由决策。
- **多播配置**：管理多播地址、多播路由表和策略。
- **统计和监控**：提供详细的网络接口和路由统计信息，可用于故障排查和性能分析。
