...
const compareVersion = (v1: string, v2: string) => {
  ...
  for (let i = 0; i < Math.max(parts1.length, parts2.length); i++) {
    const num1 = parts1[i] === 'beta' ? -1 : parseInt(parts1[i]) || 0;
    const num2 = parts2[i] === 'beta' ? -1 : parseInt(parts2[i]) || 0;
...
const sortVersioons = (versions: string[]) =>
  versions.sort((a, b) => compareVersion(b, a));

console.log('sortVersioons--->', sortVersioons(versions));
