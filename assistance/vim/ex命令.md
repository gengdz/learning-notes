...
重复上次的ex命令，使用 `@:`  在执行过一次 `@:`之后就可以使用 `@@` 来重复它啦，
...
## 命令补全
使用 `Tab` 自动补全
使用 `<C-d>` 可以显示完整列表，输入开头字母按 `Tab` 可以补全
...
使用 set history=200 可以设置历史记录为200



## 命令行窗口
`q:` 打开Ex命令历史的命令行窗口。可以做修改，然后使用 CR 执行命令，或者使用:q 退出
`<Ctrl-f>` 从命令行模式切换到命令行窗口
修改完后，按 <CR> 就会执行


`:! ls` 执行外部命令
`:shell`启动一个shell可以执行多条命令，输入exit返回vim

`:read !{cmd}`
在 shell 中执行 {cmd} ，并把其标准输出插入光标下方
