...
* s 删除光标所在字符，并进入插入模式。4s 删除前面 4个字符，并进入插入模式
...
* `<C-a>` 是递增数字，`<C-x>` 是递减数字。使用方为：10 ctl a 实现增加 10 。但是和系统的快捷键冲突，所以目前通过 leader + a 来做递增，leader + x 来做递减。
