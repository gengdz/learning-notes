...
* s 删除光标所在字符，并进入插入模式。（好像是没有效果）
* S 删除当前所在行，并进入插入模式。（好像是没有效果）
...
* M 移动到当前页的中部
* L 移动到当前页的最后一行。
* ctrl i 前进，ctrl o 后退。vim 又一个路由表
...
* ctrl-a 是递增数字，ctrl-x 是递减数字。使用方为：10 ctl a 实现增加 10 。但是和系统的快捷键冲突，所以目前通过 leader + a 来做递增，leader + x 来做递减。
...
* Ctrl + w 删除光标前的一个单词
* Ctrl + u 删除至行首
* Ctrl + o 可以在插入模式执行一次普通模式的命令，然后再回到插入模式
